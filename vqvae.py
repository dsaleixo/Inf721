...
from vitEncodeDecoder import DecoderViT, EncoderViT
from torch.utils.data import DataLoader

class VQVAE(nn.Module):
    def __init__(self, latent_dim=20, num_embeddings=10, beta=0.25, use_ema=True, ema_decay=0.99, e_width = 16, d_width = 16):
        super().__init__()
        self.encoder = EncoderViT(in_channels=15,p=4,dim=16,depth=1,latent_dim=20)
        self.decoder = DecoderViT(in_channels=15,p=4,dim=16,depth=1,latent_dim=20)
...
class TestVQVAE():
    ...
    def test0():
        ...
        for batch_idx, batch_dados in enumerate(dataloader):
            y = vqvae(batch_dados)[0]
            example = y[0].view(5,3,16,16)
            for i in range(5):
